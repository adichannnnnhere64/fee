...
test('getActiveFeeFor returns entity-specific fee when exists', function () {
...
test('getActiveFeeFor returns global fee when no entity-specific fee', function () {
    // Create global fee
...
test('getActiveFeeFor returns null when no fees exist', function () {
    $result = $this->service->getActiveFeeFor($this->user, 'product');
    
    expect($result)->toBeNull();
});

test('getActiveFeeFor only returns active fees', function () {
...
test('getActiveFeeFor respects effective dates', function () {
...
test('setFeeForEntity creates new fee and deactivates old ones', function () {
...
test('createGlobalFee creates fee with global flag', function () {
    $data = [
        'item_type' => 'product',
        'fee_type' => 'markup',
        'value' => 20.0,
...
test('calculateFor returns correct calculation for percentage fee', function () {
    // Create a fee
    FeeRule::create([
        'entity_type' => get_class($this->user),
        'entity_id' => $this->user->getKey(),
...
test('calculateFor returns correct calculation for fixed fee', function () {
    // Create a fixed fee
    FeeRule::create([
        'entity_type' => get_class($this->user),
        'entity_id' => $this->user->getKey(),
...
test('calculateFor returns no fee when none exists', function () {
    $result = $this->service->calculateFor($this->user, 100.00, 'product');
...
test('getAllActiveFeesFor returns all item types', function () {
...
test('getGlobalFees returns only global active fees', function () {
    // Create global fee
    $globalFee = FeeRule::create([
        'entity_type' => null,
        'entity_id' => null,
...
test('clearCacheForEntity removes cached fees', function () {
...
test('validation prevents invalid fee types', function () {
    expect(function () {
        FeeRule::create([
            'entity_type' => get_class($this->user),
            'entity_id' => $this->user->getKey(),
...
test('validation prevents invalid item types', function () {
    expect(function () {
        FeeRule::create([
            'entity_type' => get_class($this->user),
            'entity_id' => $this->user->getKey(),
...
test('isCurrentlyActive returns correct status', function () {
    $fee = FeeRule::create([
        'entity_type' => get_class($this->user),
        'entity_id' => $this->user->getKey(),
        'item_type' => 'product',
...
test('isCurrentlyActive returns false for future effective date', function () {
    $fee = FeeRule::create([
        'entity_type' => get_class($this->user),
        'entity_id' => $this->user->getKey(),
        'item_type' => 'product',
...
test('isCurrentlyActive returns false for expired fee', function () {
    $fee = FeeRule::create([
        'entity_type' => get_class($this->user),
        'entity_id' => $this->user->getKey(),
        'item_type' => 'product',
...
test('multiple entities can have separate fees', function () {
